...
let garagem = {};
const GARAGEM_KEY = 'garagemData_v9_apis'; // Nova versão da chave
...
function carregarGaragem() {
    ...
    if (!carregouOk || Object.keys(garagem).length === 0) { // Garante que se carregou mas está vazia, inicializa padrões
...
// ATENÇÃO: A FUNÇÃO buscarPrevisaoTempoAPI FOI REMOVIDA POIS SERÁ SUBSTITUÍDA
// PELA buscarPrevisaoDetalhada E SEU PROCESSAMENTO. SE QUISER MANTER A PREVISÃO
// SIMPLES EM OUTRO LUGAR, COPIE-A DE VOLTA DE UMA VERSÃO ANTERIOR.

// NOVO: Alerta de Segurança CRÍTICO
...
// const apiKeyPadraoOpenWeatherMap = "SUA_CHAVE_OPENWEATHERMAP_AQUI"; // <-- Removido daqui, será pego do input
 ...
 * @param {string} apiKey - A chave da API OpenWeatherMap.
 * @returns {Promise<object|null>} Um objeto com os dados da previsão da API ou null em caso de erro.
 */
async function buscarPrevisaoDetalhada(cidade, apiKey) {
    if (!cidade || !apiKey) {
        console.error("Cidade e API Key são obrigatórias para buscar previsão detalhada.");
        alert("Por favor, informe a cidade e sua API Key da OpenWeatherMap.");
        return null;
    }

    // ATENÇÃO: O ALERTA DE SEGURANÇA SOBRE A API KEY JÁ ESTÁ ACIMA E DEVE SER MANTIDO NO README.
    // A apiKey é passada como parâmetro agora, vinda do input do usuário.

    const cidadeCodificada = encodeURIComponent(cidade);
    // Endpoint para "5 day / 3 hour forecast"
    const urlAPI = `https://api.openweathermap.org/data/2.5/forecast?q=${cidadeCodificada}&appid=${apiKey}&units=metric&lang=pt_br`;
    
    console.log(`Buscando previsão detalhada para: ${cidade} em ${urlAPI}`);

    try {
        const response = await fetch(urlAPI);
        const data = await response.json(); // OpenWeatherMap geralmente retorna JSON mesmo para erros HTTP
        ...
        if (!response.ok) {
            ...
            throw new Error(`Falha ao buscar previsão: ${mensagemErroAPI}. Verifique a cidade e a API Key.`);
        }
        ...
        // Não mostre alert() aqui, deixe o chamador tratar a exibição do erro.
        throw error; // Relança o erro para ser tratado pelo event listener
    }
}
 ...
 * (data, temp_min, temp_max, descricao, icone), ou null se os dados forem inválidos.
 * Ex: [{ data: 'AAAA-MM-DD', temp_min: X, temp_max: Y, descricao: 'Céu limpo', icone: '01d' }, ...]
 */
function processarDadosForecast(data) {
    ...
    data.list.forEach(item => {
        ...
        if (!previsaoPorDia[dia]) {
            previsaoPorDia[dia] = {
                temps: [],
                weatherEntries: [], // Armazena objetos {icon, description, dt_txt}
                dt_unix_list: []  // Para ajudar a escolher o ícone do meio-dia
            };
        }
        ...
        previsaoPorDia[dia].weatherEntries.push({
            ...
            dt_txt: item.dt_txt // Guardar o dt_txt completo para escolher o do meio-dia
        });
    ...
    for (const diaStr in previsaoPorDia) {
        ...
        // Escolher uma descrição e ícone representativos (ex: o do meio-dia)
        let iconeRep = dadosDoDia.weatherEntries[0].icon;
        let descricaoRep = dadosDoDia.weatherEntries[0].description;

        // Tenta encontrar a previsão mais próxima do meio-dia (12:00)
        ...
        if (entradaMeioDia) {
            ...
            // Se não houver meio-dia, pega a entrada do meio da lista de previsões do dia
    ...
    // Ordenar por data, caso a API não garanta ou a lógica de objeto mude a ordem
...
function exibirPrevisaoDetalhada(previsaoDiariaProcessada, nomeCidade, areaResultado) {
    ...
    areaResultado.innerHTML = ''; // Limpa resultados anteriores
    ...
    titulo.innerHTML = `<i class="fa-solid fa-calendar-days"></i> Previsão para ${nomeCidade} (Próximos Dias)`;
    areaResultado.appendChild(titulo);

    const containerDias = document.createElement('div');
    containerDias.className = 'forecast-container'; // Para estilização (ex: flexbox)

    previsaoDiariaProcessada.forEach(diaInfo => {
        const diaCard = document.createElement('div');
        diaCard.className = 'day-weather-card'; // Classe para estilizar cada dia

        // Formatar data para exibição mais amigável
        // Adicionar 'T00:00:00' garante que a data seja interpretada como local sem problemas de fuso na conversão.
    ...
    // Adicionar alguns estilos básicos via JS para não depender de CSS externo para o básico
    // O ideal é ter isso num arquivo .css
    if (!document.getElementById('forecast-styles')) {
        ...
        style.innerHTML = `
            .forecast-container {
                ...
                justify-content: space-around; /* Distribui espaço ou centraliza */
                margin-top: 10px;
            }
            .day-weather-card {
                ...
                padding: 15px;
                text-align: center;
                min-width: 120px; /* Largura mínima para cada card de dia */
            ...
            .forecast-container h4 { width: 100%; text-align: center; margin-bottom: 10px;}
        `;
...
function inicializarAplicacao() {
    ...
    try {
        setupEventListeners();
        carregarGaragem(); // Isso também chama atualizarInterfaceCompleta
...
function setupEventListeners() {
    ...
    // MODIFICADO: Listener para o botão de buscar previsão do tempo
    ...
    const inputApiKeyClima = document.getElementById('viagem-api-key'); // Pega a API Key do input
    const areaResultadoPrevisao = document.getElementById('previsao-resultado-area');

    if (btnBuscarPrevisao && inputDestino && inputApiKeyClima && areaResultadoPrevisao) {
        btnBuscarPrevisao.addEventListener('click', async () => {
            const cidade = inputDestino.value.trim();
            const apiKey = inputApiKeyClima.value.trim(); // Pega a chave do input

            if (!cidade) {
                alert("Por favor, informe a Cidade de Destino.");
                exibirPrevisaoDetalhada(null, cidade, areaResultadoPrevisao); // Limpa e mostra msg
                return;
            }
            if (!apiKey) {
                alert("Por favor, informe sua API Key da OpenWeatherMap.");
                exibirPrevisaoDetalhada(null, cidade, areaResultadoPrevisao); // Limpa e mostra msg
                // Adiciona o alerta de segurança se a chave não for informada
                areaResultadoPrevisao.innerHTML += `<p style="color: yellow; font-weight: bold;">
                    <i class="fa-solid fa-triangle-exclamation"></i> Lembrete: Em um app real, a API Key NUNCA deve ser manuseada ou exposta no frontend.
                    Use um backend como proxy!
                </p>`;
                return;
            }
            ...
            try {
                ...
                const dadosApi = await buscarPrevisaoDetalhada(cidade, apiKey);
                
                if (dadosApi) {
                    // 2. Processar os dados
                    const previsaoProcessada = processarDadosForecast(dadosApi);
                    
                    // 3. Exibir os dados processados
                    exibirPrevisaoDetalhada(previsaoProcessada, dadosApi.city?.name || cidade, areaResultadoPrevisao);
                } else {
                    // buscarPrevisaoDetalhada já trata erros internos e retorna null ou lança exceção
                    // Se chegou aqui com dadosApi = null e sem exceção, é um caso não esperado, mas tratar.
                     exibirPrevisaoDetalhada(null, cidade, areaResultadoPrevisao);
                }

            } catch (error) { // Captura erros lançados por buscarPrevisaoDetalhada ou outros
        ...
        console.warn("Elementos do Planejador de Viagem (nova versão) não encontrados no DOM.");
    }
